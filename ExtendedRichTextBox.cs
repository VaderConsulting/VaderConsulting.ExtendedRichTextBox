    ...
    public class ExtendedRichTextBox : RichTextBox
    {
        ...
        public Font SelectionFont2
        {
            get
            {
                ...
                {
                    Font f = new Font(GetStringFromChar(param.szFaceName), (float)8, GraphicsUnit.Inch);
                    Font f2 = new Font(f.Name, (float)f.Size);
    ...
    */
        ...
        protected override void OnDragEnter(DragEventArgs drgevent)
        {
            ...
            else if (drgevent.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                _DropPath = a.GetValue(0).ToString();
            }
        }
        ...
        protected override void OnDragDrop(DragEventArgs drgevent)
        {
            ...
            else if (drgevent.Data.GetDataPresent(DataFormats.FileDrop))
            {
                try
                {
                    if (_DropPath.Length == 0) return;

                    if (System.IO.File.Exists(_DropPath))
                    {
                        System.IO.FileInfo fi = new System.IO.FileInfo(_DropPath);
                        if (fi.Extension.ToLower() == ".rtf")
                        {
                            this.LoadFile(_DropPath); drgevent.Data.SetData(null); drgevent = null; return;
                        }
                        else if (IsImageExtension(fi.Extension.ToLower()))
                        {
                            this.InsertImage(_DropPath); drgevent.Data.SetData(null); drgevent = null; return;
                        }
                    }
                }
                catch (Exception) { }
